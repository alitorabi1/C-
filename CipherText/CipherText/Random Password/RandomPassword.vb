...
#Region " Methods "
    ...
    ' no longer than maximum default.
    ' </remarks>
    Public Shared Function Generate(ByVal length As Integer, ByVal bolIncludeSpecialCharacters As Boolean, ByVal bolIncludeNumbers As Boolean, ByVal bolIncludeUpperCase As Boolean, ByVal bolIncludeLowerCase As Boolean) As String

        If bolIncludeLowerCase = False AndAlso bolIncludeNumbers = False AndAlso bolIncludeSpecialCharacters = False AndAlso bolIncludeUpperCase = False Then
            Return "tryagain"
        End If

        PASSWORD_CHARS_LCASE = "abcdefgijkmnopqrstwxyz"
        PASSWORD_CHARS_UCASE = "ABCDEFGHJKLMNPQRSTWXYZ"
        PASSWORD_CHARS_NUMERIC = "23456789"
        PASSWORD_CHARS_SPECIAL = "!@#$"

        Dim strReplacement As String = String.Empty

        If bolIncludeLowerCase = True Then
            strReplacement = PASSWORD_CHARS_LCASE

        ElseIf bolIncludeUpperCase = True Then
            strReplacement = PASSWORD_CHARS_UCASE

        ElseIf bolIncludeNumbers = True Then
            strReplacement = PASSWORD_CHARS_NUMERIC

        ElseIf bolIncludeSpecialCharacters = True Then
            strReplacement = PASSWORD_CHARS_SPECIAL
        End If

        If bolIncludeSpecialCharacters = False Then
            PASSWORD_CHARS_SPECIAL = strReplacement

        Else
            PASSWORD_CHARS_SPECIAL = "!@#$"
        End If

        If bolIncludeNumbers = False Then
            PASSWORD_CHARS_NUMERIC = strReplacement

        Else
            PASSWORD_CHARS_NUMERIC = "23456789"
        End If

        If bolIncludeUpperCase = False Then
            PASSWORD_CHARS_UCASE = strReplacement

        Else
            PASSWORD_CHARS_UCASE = "ABCDEFGHJKLMNPQRSTWXYZ"
        End If

        If bolIncludeLowerCase = False Then
            PASSWORD_CHARS_LCASE = strReplacement
